...
class subject:
    ...
    def __repr__(self):
        return 'subject({}, {}, {}, {}, {})'.format(self.name.__repr__(), self.short_name.__repr__(), self.credits,
                                                    self.lab, self.subcode.__repr__())
...
class ParentWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.systemtray_icon = QSystemTrayIcon(QIcon('icons/logo.ico'))
    ...
    def setup_first_window(self):
        ...
        self.sem_list = ['I','II','III', 'IV', 'V', 'VI', 'VII', 'VIII']
        ...
        self.desig_list = ['Lecturer','Guest Faculty','Assistant Professor', 'Professor', 'Associate Professor', 'Head of Department', 'Principal']
    ...
    def setup_about_window(self):
        ...
        self.ui_about.logoLabel.setPixmap(QtGui.QPixmap('icons/logo.ico'))
    ...
    def setup_license_window(self):
        ...
        self.ui_license.logoLabel.setPixmap(QtGui.QPixmap('icons/logo.ico'))
    ...
    def continue_btn_event(self):
        ...
        logger.info('Academic year: %s %s %s %s Department: %s', self.startMonth, self.startYear, self.endMonth,
                    self.endYear, self.department)
    ...
    def section_spinbox_event(self):  # function for sections spinbox
        if self.sem != '':
            self.num_sections[
                self.sem] = self.ui.sections_spinbox.value()  # saves the number of sections when "add" button is clicked
        ...
        if self.row:  # when editing a currently selected value in list
                    ...
                    for y in self.subjects[sem]:
                        if subj == y:
                            ...
                            if text != '':
                                ...
                                y.lab = lab
                                y.subcode = subcode
                                logger.info('Subject details(name, shortname, credits, lab) :%s %s %s %s', text,
                                            short_sub, credits, lab)
                                t = text + " - " + short_sub
                ...
                for x in self.row:
                    ...
                    for y in self.faculty_list_value:
                        if fac == y.name:
                            ...
                            logger.debug('Faculty details(title, name, designation): %s %s %s', y.title, y.name,
                                         y.designation)
                            break
            self.ui.input_list.clearSelection()
            self.row = []
            ...
            if self.inputType == "Subjects":
                ...
                if text != '':
                    ...
                    logger.info('Subject details(name, shortname, credits, lab) :%s %s %s %s', text, short_sub, credits,
                                lab)
                    t = text + " - " + short_sub
    ...
    def add_btn_elective_event(self):
        ...
        if row:  # when editing a currently selected value in list
                ...
                for y in self.electives[sem][elective_group]:
                    if subj == y:
                        if text != '':
                            ...
                            y.lab = lab
                            y.subcode = subcode
                            logger.info('Elective details(name, shortname, credits, lab) :%s %s %s %s', text, short_sub,
                                        credits, lab)
                            t = text + " - " + short_sub
            ...
            if text != "":
                ...
                lab = self.ui_elec.lab_checkbox.isChecked()
                sub = subject(text, short_sub, credits, lab, subcode)
                logger.info('Elective details(name, shortname, credits, lab) :%s %s %s %s', text, short_sub, credits,
                            lab)
                t = text + " - " + short_sub
                for group in self.electives[sem]:
    ...
    def assign_btn_event(self):
        ...
        for i, x in enumerate(self.subjects_assigned[sem][section]):
            ...
            if subj == sub:
                try:
                    if self.subs[x[
                        1]].lab == False or faculty in teachers:  # if theory subject or teacher already assigned to subject
    ...
    def semester_combobox3_event(
            self):  # load the number of sections in sec combobox and load the slot allotments made when semester is changed
    ...
    def cellClick3_event(self, row, column):
        ...
        dict_entry_exists = sem in self.section_fixed_slots and section in self.section_fixed_slots[sem] and row in \
                            self.section_fixed_slots[sem][section] and column in self.section_fixed_slots[sem][section][
                                row]
    ...
    def cellClick4_event(self, row, column):
        ...
        faculty = self.ui4.faculty_combobox.currentText()
        if faculty in self.faculty_fixed_slots and row in self.faculty_fixed_slots[faculty] and column in \
                self.faculty_fixed_slots[faculty][row] and self.faculty_fixed_slots[faculty][row][column] == '-':
                    ...
                    if faculty in self.faculty_timetables:
                        ...
                        if cur_sub != '':
                            ...
                            if self.timetables[cur_sem][cur_sec][prev_d][prev_t]:
                                can_swap = False
                                conflicts.append((cur_sem + ' ' + cur_sec, prev_d, prev_t,
                                                  self.timetables[cur_sem][cur_sec][prev_d][prev_t][3]))
                        ...
                        if prev_sub != '':
                            ...
                            if self.timetables[prev_sem][prev_sec][cur_d][cur_t]:
                                can_swap = False
                                conflicts.append((prev_sem + ' ' + prev_sec, cur_d, cur_t,
                                                  self.timetables[prev_sem][prev_sec][cur_d][cur_t][3]))
                        else:
                            prev_cell_text = '-'
                        ...
                        if can_swap:
                            ...
                            f[cur_d][cur_t], f[prev_d][prev_t] = f[prev_d][prev_t], f[cur_d][
                                cur_t]  # swap in faculty timetable
                            if prev_sub:
    ...
    def print_btn_event(self):
        ...
        if inputType == 'Students':
            ...
            if sem == '' or sec == '':
                ...
                try:
                    year = '{}. {} - {}. {}'.format(self.startMonth[:3], self.startYear, self.endMonth[:3],
                                                    self.endYear)
                    worddoc.make_docx(tt, 'section', filepath, self.subjects_assigned[sem][sec], self.subs,
                                      self.faculty_list_value, year)
                    os.startfile(filepath)
                except OSError as err:
                    self.show_printerror_dialog(err)
                    logger.exception(err)
            ...
            if faculty == '':
                ...
                try:
                    ...
                    worddoc.make_docx(tt, 'faculty', filepath, self.faculty_subjects[faculty], self.subs,
                                      self.timetables, designation)
                    os.startfile(filepath)
                except OSError as err:
                    self.show_printerror_dialog(err)
                    logger.exception(err)
    ...
    def finish_btn_event(self, event):
        ...
        buttonReply = QMessageBox.warning(self, 'EXIT Confirmation',
    ...
    def filemenuevent(self, option):
        ...
        if option == "Exit":
            ...
            buttonReply = QMessageBox.question(self, 'Load Confirmation',
            ...
            buttonReply = QMessageBox.question(self, 'Reset Confirmation',
            ...
            try:
                ...
                for faculty in self.faculty_timetables:
                    tt = self.faculty_timetables[faculty]
                    filepath = os.path.join('Output', 'Personal Timetables',
    ...
    def show_save_file_dialog(self):
        ...
        while dialog.exec_():
            ...
            if os.path.isfile(fname):
                buttonReply = QMessageBox.question(self, 'Save confirmation',
    ...
    def load_excel(self, fname):
        try:
            ...
            electives = {'I': dict(),'II': dict(),'III': dict(), 'IV': dict(), 'V': dict(), 'VI': dict(), 'VII': dict(), 'VIII': dict()}
            subjects_assigned = OrderedDict(
                {'I': OrderedDict(),'II': OrderedDict(),'III': OrderedDict(), 'IV': OrderedDict(), 'V': OrderedDict(), 'VI': OrderedDict(),
                 'VII': OrderedDict(), 'VIII': OrderedDict()})
            faculty_subjects = dict()
            sections = OrderedDict({'I':[],'II':[],'III': [], 'IV': [], 'V': [], 'VI': [], 'VII': [], 'VIII': []})
            cur_sem = ''
            cur_sub = ''
            cur_sec = 1
...
if __name__ == "__main__":
    ...
    global loc
    loc = os.path.realpath(os.curdir) + '\\' + os.path.join('logs', time.strftime("%a, %d %b %Y %H-%M-%S.log",
                                                                                  time.localtime()))
    my_logger()
    ...
    app.setApplicationName('TIME CRAFT')
    main = ParentWindow()
