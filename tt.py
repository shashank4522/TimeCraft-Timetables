...
def generate(tt, subjects, faculty,
             location):  # subjects is a list of tuples (name, hours/week, teacher, short name); faculty is a dict
        ...
        for i in range(len(subjects)):  # iterate through all subjects, allot 1 hour for each subject
            if remaining_hours[i] > 0:
                ...
                for day, time in freeslots:
                    if faculty[subject[2]].final[day][time] != True and getnumhours(tt, subject, day)[
                        0] < 1:  # if teacher slot is not finalized and subject isnt already there that day
                        tt[day][time] = subject
...
def print_timetable(tt, location, style='section', name=''):
    ...
    if name == '':
        name = tt.name
    logger.info(('%-20s ' * 9) % (
    name, '9:00-9:55', '9:55-10:50', '11:10-12:05', '12:05-1:00', '1:00-1:55', '1:55-2:50', '2:50-3:40', '3:40-4:30'))
    for day in tt:
        x = '%-20s' % day
        for timeslot in tt[day]:
            if tt[day][timeslot] == '':
...
def rehabilitate(day, section, subject, faculty):
    ...
    for timeslot in teacher[day]:
        if teacher[day][timeslot] == '' and teacher.final[day][timeslot] != True and not is_consecutive_hour(teacher,
                                                                                                             day,
                                                                                                             timeslot) and \
                getnumhours(section, subject, day)[0] < 2:  # teacher is available
            if section.final[day][timeslot] != True:  # time slot for that section is not finalized
                clashing_subject = section[day][timeslot]
                if teacher.workload > section.final[day][
                    timeslot]:  # whatever subject has been allotted, move it to clash
                    if clashing_subject != '':
                        clash.append((section, clashing_subject))
                        clashing_teacher = clashing_subject[2]
                        faculty[clashing_teacher][day][timeslot] = ''
                    section.final[day][
                        timeslot] = teacher.workload  # finalize the lecture by moving the new subject into the time slot
            ...
            if tt[day][t] == '' and tt.final[day][t] == False:
                for i in range(t2, t, -1):  # i = future timeslot that has a lecture
                    if tt[day][i] != '' and tt.final[day][i] != True:
                        subject = tt[day][i]
                        teacher = faculty[subject[2]]
                        if teacher[day][t] == '' and teacher.final[day][t] == False and not is_consecutive_hour(teacher,
                                                                                                                day, t):
        ...
        for timeslot in range(1, timeslots + 1):
            for sem in timetables:
                for section in timetables[sem]:
                    ...
                    if section[day][timeslot] != '' and section.final[day][
                        timeslot] == False:  # if lecture has not been finalized
                        subject = section[day][timeslot]
                        teacher = faculty[subject[2]]

                        try:
                            if teacher[day][timeslot] == '' and teacher.final[day][
                                timeslot] != True and not is_consecutive_hour(teacher, day, timeslot):
                                teacher[day][timeslot] = (section.name, subject)
                                section.final[day][timeslot] = teacher.workload
                            elif teacher[day][timeslot] != '' and (
                                    teacher[day][timeslot][0] != section.name or teacher[day][timeslot][
                                1] != subject):  # teacher is not available, there is a clash
                            ...
                            logger.info('%s %s %s %s %s', section.name, subject, day, timeslot,
                                        faculty[subject[2]][day][timeslot])
...
def finalize_elective(section, day, time, subjects, sub_short):
    global faculty
    section[day][time] = (
    'Elective', 0, 'Elective staff', sub_short)  # last field is the one that matters, others are not used
...
if __name__ == '__main__':
    ...
    location = os.path.realpath(
        os.curdir) + '\example.log'  # change this string to create log when running tt.py as a standalone script
    logging.basicConfig(filename=location, level=logging.INFO)
